...
function util.schedule_function(f, ...)
   local pack = table.pack or function(...) return { n = select("#", ...), ... } end --! FOR TESTS TO PASS
...
function util.get_default_rockspec()
   ...
   if #unnamed_paths > 0 then
      ...
      if rock then
         ...
         if next(versions, rock) ~= nil then
            return nil, "Please specify which rockspec file to use."
         else
            return paths[rock]
         end
...
function util.opts_table(type_name, valid_opts) --! TEMP
   local opts_mt = {}
